...
public abstract class AbstractSendChannel<E> implements SendChannel<E> {
    ...
    */
    ...
    public final Object mo1849trySendJP2dKIU(E element) {
        Object objOfferInternal = offerInternal(element);
        if (objOfferInternal == AbstractChannelKt.OFFER_SUCCESS) {
            return ChannelResult.INSTANCE.m1869successJP2dKIU(Unit.INSTANCE);
        }
        if (objOfferInternal == AbstractChannelKt.OFFER_FAILED) {
            Closed<?> closedForSend = getClosedForSend();
            return closedForSend == null ? ChannelResult.INSTANCE.m1868failurePtdJZtk() : ChannelResult.INSTANCE.m1867closedJP2dKIU(helpCloseAndGetSendException(closedForSend));
        }
        if (objOfferInternal instanceof Closed) {
            return ChannelResult.INSTANCE.m1867closedJP2dKIU(helpCloseAndGetSendException((Closed) objOfferInternal));
        }
    ...
    public final void helpCloseAndResumeWithSendException(Continuation<?> continuation, E e, Closed<?> closed) {
        ...
        if (function1 != null && (undeliveredElementExceptionCallUndeliveredElementCatchingException$default = OnUndeliveredElementKt.callUndeliveredElementCatchingException$default(function1, e, null, 2, null)) != null) {
            ...
            continuation.resumeWith(Result.m299constructorimpl(ResultKt.createFailure(undeliveredElementException)));
            return;
        }
        Result.Companion companion2 = Result.INSTANCE;
        continuation.resumeWith(Result.m299constructorimpl(ResultKt.createFailure(sendException)));
    }
    ...
    private final void helpClose(Closed<?> closed) {
        ...
        while (true) {
            ...
            if (receive == null) {
                break;
            } else if (!receive.mo1894remove()) {
                receive.helpRemove();
            } else {
                objM1886constructorimpl$default = InlineList.m1891plusFjFbRPM(objM1886constructorimpl$default, receive);
            }
        }
        if (objM1886constructorimpl$default != null) {
            if (!(objM1886constructorimpl$default instanceof ArrayList)) {
                ((Receive) objM1886constructorimpl$default).resumeReceiveClosed(closed);
            } else if (objM1886constructorimpl$default != null) {
                ArrayList arrayList = (ArrayList) objM1886constructorimpl$default;
